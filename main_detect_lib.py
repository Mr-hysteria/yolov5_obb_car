...
def move_1():
    ...
    A = camera_coordinate_4d
        ...
        #             robot_pos[5]]  # 小车上
    position = [robot_pos[0] - A[0], robot_pos[1], robot_pos[2] - A[1] - 10, robot_pos[3], robot_pos[4],robot_pos[5]]  # 小车上2
...
def move_2():
    ...
    while True:
        ...
        if pos[5]>70:
	        position = [A[0], A[1] + 110, A[2]-25, pos[3], pos[4], pos[5]]
        else:
	        position = [A[0]-20, A[1] + 110, A[2]-25, pos[3], pos[4], pos[5]]  # 小车上2
        erobot.set_coords(position, 2000)
...
def realsense_detect():  # 进行目标识别，显示目标识别效果，返回相机坐标系下的值
    ...
    try:
        while True:
            ...
            if many_time%15==0 or many_time==1:
                move_1()
...
while numbers1 >= 1:
    ...
    # erobot.set_angles([50,-85,71,-168,-75,60], 1080)
    time.sleep(1)
    erobot.wait_command_done()
